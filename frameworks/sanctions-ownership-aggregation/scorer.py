#!/usr/bin/env python3
"""CLI for aggregate sanctioned-ownership resolution."""
...
from _lib.sanctions_ownership import SanctionsConfig, resolve_graph
...
def score_payload(payload: dict) -> dict:
    ...
    for index, graph in enumerate(graphs):
        ...
        config = SanctionsConfig(
            blocked_threshold=float(graph.get("blocked_threshold", payload.get("blocked_threshold", .50))),
            review_floor=float(graph.get("review_floor", payload.get("review_floor", .25))),
            near_threshold_margin=float(graph.get("near_threshold_margin", payload.get("near_threshold_margin", .02))),
        ...
        output["results"].append({"graph_id": graph.get("graph_id", f"graph-{index}"), "candidates": results})
    return output
