...
- **14 runnable engines.** Small, transparent calculators for the problems that are
...
| **[`frameworks/`](frameworks/)** | **14 runnable scoring engines with validation evidence** — pure-stdlib reference engines for the financial-crime problems that are a measurable scoring/triage/matching problem (sanctions and PEP screening, transaction monitoring, threshold tuning, customer risk rating, jurisdiction risk, adverse media, on-chain KYT, on-chain OSINT evidence, watchlist knowledge base, case QA, NPA product risk, data-quality rules, attribute sampling), each with a seeded synthetic-data generator and a harness that emits reproducible accuracy evidence (recall, false-positive reduction, threshold sweeps) and fails the build on a false-negative-safety breach. A different artifact class from the paste-prompts; runnable, multi-file. |
...
- **Evidence, not assertion.** Every number in every `evidence/` pack is emitted by a harness, and CI **re-derives all fourteen packs from seed on every commit** and fails if one differs. Safety claims carry an exact confidence bound, not just "recall 1.0". Check it yourself in twenty seconds: `python3 _tooling/verify_evidence.py`. See [`frameworks/EVIDENCE.md`](frameworks/EVIDENCE.md).
