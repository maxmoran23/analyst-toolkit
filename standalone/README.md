...
Every file in this directory is a **complete, self-contained instruction set**. Copy one whole file into any AI assistant (GitHub Copilot, Microsoft 365 Copilot, Claude, ChatGPT, or anything comparable), reply with your inputs when asked, and the assistant performs the analysis to a defensible standard. Most need no repository companion. The three artifact-preservation workflows are explicit exceptions: `data-to-dashboard.md` may use one supplied HTML shell, `material-to-deck.md` pairs the semantic plan with a supplied branded deck and the placeholder-only injector, and `dashboard-ai-assistant.md` pairs your existing dashboard with the supplied AI analyst component file.
...
| [dashboard-ai-assistant.md](dashboard-ai-assistant.md) | An existing single-file dashboard retrofitted with an in-dashboard AI analyst sidebar (from the supplied component file) — deterministic query + insight engine over the dashboard's own data, clarifies instead of guessing, refuses honestly, generates handoff packs for a full AI assistant |
